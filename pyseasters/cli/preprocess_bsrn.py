...
"""
Entry point for the ``preprocess_bsrn`` command. The command has a help option:

.. code-block:: console

    $ preprocess_bsrn --help
    usage: preprocess_bsrn [-h] [-n NTASKS] [-m MEMORY_PER_TASK] [-v] [-s] [-f]

    Preprocess BSRN data files.
...
from pyseasters.data_curation import preprocess_bsrn
from pyseasters.utils._logging import setup_cli_logging


def main():
    parser = argparse.ArgumentParser(description="Preprocess BSRN data files.")
    parser.add_argument(
        "-n",
        "--ntasks",
        type=int,
