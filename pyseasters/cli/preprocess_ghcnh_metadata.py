...
"""
Entry point for the ``preprocess_ghcnh_metadata`` command.
The command has a help option:

.. code-block:: console

    $ preprocess_ghcnh_metadata --help
    usage: preprocess_ghcnh_metadata [-h] [-v] [-s] [-f]

    Preprocess GHCNh metadata files (filter countries, simplify inventory and compress).
...
from pyseasters.data_curation import preprocess_ghcnh_metadata
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Preprocess GHCNh metadata files "
            + "(filter countries, simplify inventory and compress)."
        )
    )
    parser.add_argument(
