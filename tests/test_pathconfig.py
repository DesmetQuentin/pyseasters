...
def test_missing_paths_yaml(caplog):
    ...
    original_machine_to_path = pyseasters.constants.pathconfig.MACHINE_TO_PATH.copy()

    with patch("importlib.resources.files", side_effect=FileNotFoundError):
        # Reload the module to trigger the import
        with caplog.at_level(logging.WARNING):
            importlib.reload(pyseasters.constants.config)
    ...
    pyseasters.constants.pathconfig.MACHINE_TO_PATH.clear()
    pyseasters.constants.pathconfig.MACHINE_TO_PATH.update(original_machine_to_path)
...
def test_initialize_with_known_machine(config, monkeypatch):
    ...
    original_machine_to_path = pyseasters.constants.pathconfig.MACHINE_TO_PATH.copy()
    pyseasters.constants.pathconfig.MACHINE_TO_PATH.update(
        {"KNOWN MACHINE": "/known/directory"}
    )
    monkeypatch.setattr("pyseasters.pathconfig.CURRENT_MACHINE", "KNOWN MACHINE")
    config.initialize()
    print(pyseasters.constants.pathconfig.MACHINE_TO_PATH)
    assert (
        config.root == pyseasters.constants.pathconfig.MACHINE_TO_PATH["KNOWN MACHINE"]
    )
    pyseasters.constants.pathconfig.MACHINE_TO_PATH.clear()
    pyseasters.constants.pathconfig.MACHINE_TO_PATH.update(original_machine_to_path)
...
def test_initialize_with_unknown_machine(config, monkeypatch):
    ...
    monkeypatch.setattr("pyseasters.pathconfig.CURRENT_MACHINE", "unknown_machine")
    with pytest.raises(RuntimeError):
        config.initialize()
