...
SEASTERSdb relies on. It is structured as follows:
...
The database file structure is not generated by SEASTERSdb at any point.
Thus, it must be generated manually prior to following anything else in this guide.

**i.** First, choose a data directory
(e.g., ``/your/working/directory/SEASTERSdb_DATA``;
...
2. Link SEASTERSdb with the data
--------------------------------

You have installed SEASTERSdb using the :doc:`installation guide <../install>`,
and you have created the correct data structure in a chose directory.
You now need to let SEASTERSdb know where is your data.
...
.. code:: python
   ...
   directory where the SEASTERSdb Python package looks at for loading data.
   ...
   if __name__ == "__main__":
       ...
       # Write data directory in 'seastersdb/constants/data/path.txt'
...
If you now run this script in an environment where SEASTERSdb is installed,
...
.. note::
   ...
   Concretely, SEASTERSdb reads the data directory root in a one-line ``txt`` file
   located at ``<seastersdb>/constants/data/path.txt``
   (``<seastersdb>`` referring to the package location,
...
SEASTERSdb command-line interface (CLI) provides the ``generate_download_script``
...
SEASTERSdb API does not deal with the raw data,
