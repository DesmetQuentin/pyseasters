"""Loaders for GHCNd data, with filters and several output format options.

GHCNd data contains for each record not only the record value but also "attributes",
i.e., additional information about the context of the measurements, how to interpret
the data, etc.
The meaning of attributes is explained in GHCNd's documentation.

This module essentially provides two types of functions: ``load_ghcnd*()`` and
``load_ghcnd_single_var_station*()``. Each has an ``attrs``, ``noattrs`` and a default
...
from seastersdb.utils._args import localize_time_range
...
def load_ghcnd_concat(
    ...
    .. attention::
       ...
       :func:`~seastersdb.ghcnd.metadata_loaders.search_ghcnd` to get a
