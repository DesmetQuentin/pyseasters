...
from seastersdb.ghcnh import load_ghcnh_inventory, load_ghcnh_station_list
from seastersdb.ghcnh._constants import ATTRIBUTES, VAR_TO_META
from seastersdb.utils._logging import LoggingStack, LoggingStackPickle
from seastersdb.utils._typing import LoggerLike
...
def _export_variables(
    ...
    var_to_count = {}
    for var in VAR_TO_META.keys():
        cols = [var] + [f"{var}{attr}" for attr in ATTRIBUTES]
...
def _preprocess_single_psv_station(
    ...
    try:
        ...
        col_dtype.update(
            {
                f"{var}{attr}": str
                for var, attr in itertools.product(VAR_TO_META.keys(), ATTRIBUTES)
            }
        )
...
def preprocess_ghcnh(
    ...
    with (
        importlib.resources.files("seastersdb.data_curation.data")
    ...
    for var, year in itertools.product(
        VAR_TO_META.keys(), station_year["year"].drop_duplicates().to_list()
    ):
