...
'seastersdb/constants/data/'. This file should contain a single line indicating
...
.. code:: python
   ...
   import seastersdb
   seastersdb.paths.manual_config(root="<path/to/database/root>")
"""
...
from seastersdb.utils._typing import PathLike
...
def _parse_pathsyaml():
    ...
    try:
        with (
            importlib.resources.files("seastersdb.constants.data")
            .joinpath("paths.yaml")
            .open("r") as file
        ):
...
class PathConfig:
    ...
    * ``init_mode == "preconfig"``:
      ...
      stored in the 'path.txt' file located in 'seastersdb/constants/data/'. If the
